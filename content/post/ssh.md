...
+++
**Release candidate version 6.3.1:** the RsyncOSX function for assisting in setting up ssh is changed. The new feature does not change anything for current users, but enables users to set their own ssh keypath and identity file. It is advised to utilize the release candidate if you want RsyncOSX to assist in setting up passwordless logins by ssh keys.
...
### Version 6.3.0 (use version 6.3.1 release candidate)
